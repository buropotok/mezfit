...
export async function validateTelegramInitData(
  ...
  if (!timingSafeEqualHex(expectedHash, suppliedHash.toLowerCase())) throw new TelegramAuthError('Telegram init data signature is invalid');

  const authDateSeconds = Number(params.get('auth_date'));
  if (!Number.isFinite(authDateSeconds) || authDateSeconds <= 0) throw new TelegramAuthError('Telegram auth_date is invalid');

  const authDate = new Date(authDateSeconds * 1000);
  const ageSeconds = (now.getTime() - authDate.getTime()) / 1000;
  if (ageSeconds < -60 || ageSeconds > maxAgeSeconds) throw new TelegramAuthError('Telegram init data is expired');
  ...
  if (!Number.isSafeInteger(user.id) || user.id <= 0 || !user.first_name) throw new TelegramAuthError('Telegram user payload is incomplete');
