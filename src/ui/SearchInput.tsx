...
export const SearchInput = forwardRef<HTMLInputElement, SearchInputProps>(function SearchInput({ className = '', value, defaultValue, onChange, onFocus, onBlur, onClear, clearLabel = 'Очистить поиск', disabled, ...props }, ref) {
  ...
  return <div className={`ui-search-input${focused ? ' ui-search-input--focused' : ''}${disabled ? ' ui-search-input--disabled' : ''} ${className}`.trim()}>
    ...
    <button type="button" className={`ui-search-input__clear${canClear && canClearValue ? ' ui-search-input__clear--visible' : ''}`} aria-label={clearLabel} tabIndex={canClear && canClearValue ? 0 : -1} aria-hidden={!(canClear && canClearValue) || undefined} disabled={!(canClear && canClearValue)} onPointerDown={(event) => { if (canClear && canClearValue) startPressScale(event.currentTarget); }} onKeyDown={(event) => { if (canClear && canClearValue && isPressScaleActivationKey(event.key)) startPressScale(event.currentTarget); }} onMouseDown={(event) => event.preventDefault()} onClick={() => { if (value === undefined) setUncontrolledValue(''); onClear?.(); }}><svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="m7 7 10 10M17 7 7 17" /></svg></button>
  </div>;
});
