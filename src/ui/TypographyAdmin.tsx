...
function cssFor(value: TypographyValue) {
  return cssDeclarations(value).replace(/^  /gm, '');
}
...
export function ComponentTypographySettings({ slots, assignments, values, onChange }: { slots: readonly TypographySlot[]; assignments: TypographyAssignments; values: TypographyValues; onChange: (assignments: TypographyAssignments) => void }) {
  ...
  const css = slots.map((slot) => {
    const role = assignments[slot.id] ?? slot.defaultRole;
    const value = slot.weightOverride === undefined ? values[role] : { ...values[role], weight: slot.weightOverride };
