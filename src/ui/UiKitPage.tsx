...
export function UiKitPage() {
  ...
  return (
    <main className="ui-kit-page">
      ...
      <Surface as="section" className="ui-kit-section"><Text variant="title">Menu</Text><Divider /><div style={{ position: 'relative', width: '44px', height: '44px' }}><button ref={menuButtonRef} type="button" className="ui-icon-button" aria-label="Open menu" aria-haspopup="menu" aria-expanded={menuOpen} onClick={() => setMenuOpen(true)}><span aria-hidden="true">⋮</span></button><Menu isOpen={menuOpen} onClose={() => setMenuOpen(false)} returnFocusRef={menuButtonRef} label="Example menu"><MenuItem leading={<span>ⓘ</span>}>Информация</MenuItem><MenuItem active leading={<span>★</span>}>Избранное</MenuItem><MenuDivider /><MenuItem leading={<span>✎</span>}>Редактировать</MenuItem></Menu></div></Surface>
