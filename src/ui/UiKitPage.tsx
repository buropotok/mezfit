...
import { Avatar, Button, Divider, FloatingActionButton, IconButton, List, ListItem, Menu, MenuDivider, MenuItem, Modal, SearchInput, SortableList, Surface, Tabs, TabsContent, TabsList, TabsTrigger, Text, type SortableListItem } from './index';
import './catalog.css';
...
export function UiKitPage() {
  ...
  return (
    <main className="ui-kit-page">
      ...
      <Surface as="section" className="ui-kit-section"><Text variant="title">Search</Text><Text variant="caption" tone="muted">Telegram Web A geometry: 40px field, 22px radius, leading search icon, focus accent and trailing clear action.</Text><Divider /><SearchInput value={searchValue} onChange={(event) => setSearchValue(event.currentTarget.value)} onClear={() => setSearchValue('')} placeholder="Поиск упражнений" aria-label="Поиск упражнений" /></Surface>
