...
export function UiKitPage() {
  ...
  return (
    <main className="ui-kit-page">
      ...
      <Surface as="section" className="ui-kit-section"><Text variant="title">Menu</Text><Divider /><Menu isOpen={menuOpen} onClose={() => setMenuOpen(false)} label="Example menu" trigger={<button type="button" className="ui-icon-button" aria-label="Open menu" onClick={() => setMenuOpen(true)}><span aria-hidden="true">⋮</span></button>}><MenuItem leading={<span>ⓘ</span>}>Информация</MenuItem><MenuItem active leading={<span>★</span>}>Избранное</MenuItem><MenuDivider /><MenuItem leading={<span>✎</span>}>Редактировать</MenuItem></Menu></Surface>
