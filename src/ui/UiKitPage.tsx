...
export function UiKitPage() {
  ...
  return (
    <main className="ui-kit-page">
      ...
      <Surface as="section" className="ui-kit-section"><Text variant="title">Menu</Text><Divider /><div style={{ position: 'relative', width: '44px', height: '44px' }}><IconButton ref={menuButtonRef} label="Open menu" onClick={() => setMenuOpen(true)}><span aria-hidden="true">⋮</span></IconButton><Menu isOpen={menuOpen} onClose={() => setMenuOpen(false)} returnFocusRef={menuButtonRef} label="Example menu"><MenuItem leading={<span>ⓘ</span>}>Информация</MenuItem><MenuItem active leading={<span>★</span>}>Избранное</MenuItem><MenuDivider /><MenuItem leading={<span>✎</span>}>Редактировать</MenuItem></Menu></div></Surface>
