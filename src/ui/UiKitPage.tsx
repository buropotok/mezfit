...
export function UiKitPage() {
  ...
  return (
    <main className="ui-kit-page">
      ...
      <Modal isOpen={modalDemo === 'default'} title="Пригласить клиента" onClose={closeModal}><Text>Здесь будет ссылка-приглашение и действие копирования.</Text></Modal>
      <Modal isOpen={modalDemo === 'info'} variant="alert" title="Готово" onClose={closeModal} actions={[{ id: 'ok', label: 'OK', onClick: closeModal }]}><Text>Изменения сохранены.</Text></Modal>
      <Modal isOpen={modalDemo === 'confirm'} variant="alert" title="Завершить тренировку?" onClose={closeModal} actions={[{ id: 'confirm', label: 'Завершить', onClick: closeModal }, { id: 'cancel', label: 'Отмена', onClick: closeModal }]}><Text>После завершения результаты будут сохранены.</Text></Modal>
      <Modal isOpen={modalDemo === 'destructive'} variant="alert" title="Удалить тренировку?" onClose={closeModal} actions={[{ id: 'delete', label: 'Удалить', tone: 'danger', onClick: closeModal }, { id: 'cancel', label: 'Отмена', onClick: closeModal }]}><Text>Это действие нельзя отменить.</Text></Modal>
    </main>
  );
}
