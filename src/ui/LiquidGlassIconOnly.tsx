...
export type LiquidGlassIconOnlyStartupItem = {
  value: string;
  icon: UiIconPair;
};
...
type LiquidGlassIconOnlyStartupArgs = {
  enabled: boolean;
  hidden: boolean;
  items: readonly LiquidGlassIconOnlyStartupItem[];
  activeValue?: string;
...
export function useLiquidGlassIconOnlyStartup({
  enabled,
  hidden,
  items,
  activeValue,
  ...
  const overlay: ReactNode = enabled ? (
    <div className="ui-tabs__icon-only-startup" aria-hidden="true">
          ...
          <div ref={maskRef} className="ui-tabs__icon-only-startup-mask">
            ...
            >
              {items.map((item, index) => (
                <span
                  key={item.value}
                  className={`ui-tabs__icon-only-startup-item${index === activeIndex ? ' is-active' : ''}`}
                >
                  <span className="ui-tabs__icon-only-startup-icon ui-tabs__icon-only-startup-icon--outline">{item.icon.outline}</span>
                  <span className="ui-tabs__icon-only-startup-icon ui-tabs__icon-only-startup-icon--filled">{item.icon.filled}</span>
                </span>
              ))}
              <span
                className="ui-tabs__icon-only-startup-selector"
