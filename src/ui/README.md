...
`--ui-color-modal` is the semantic overlay-surface color and maps to the theme-owned `--theme-modal` token. `Modal` always uses it. `BottomSheet` uses it by default through `modalColor={true}`; set `modalColor={false}` only when the sheet intentionally needs the ordinary surface color. Feature code must not hard-code modal/sheet colors.
