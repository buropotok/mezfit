...
`Tabs theme="liquidGlass" mode="iconOnly"` is the icon-only Liquid Glass tab bar variant. It requires the `hidden` prop. `hidden={true}` keeps the tab bar visually hidden and non-interactive; only the transition from `true` to `false` plays the approved startup reveal animation. Mounting with `hidden={false}` renders the final tab bar immediately. The variant owns the approved icon-only hold, drag, selector/lens movement, container scaling, highlight, spring, and optical behavior; the existing `mode="icon"` Liquid Glass mechanics are unchanged. `TabsTrigger` labels remain required for accessibility but are visually hidden in `iconOnly` mode.
