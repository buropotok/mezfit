...
`Tabs theme="liquidGlass" mode="iconOnly"` is the icon-only Liquid Glass tab bar variant. It requires the `hidden` prop. `hidden={true}` keeps the tab bar visually hidden and non-interactive; only the transition from `true` to `false` plays the startup reveal animation. Mounting with `hidden={false}` renders the final tab bar immediately. After the reveal, the variant uses the existing Liquid Glass tabs controller for hold, drag, selector/lens movement, container scaling, highlight, and optical effects. `TabsTrigger` labels remain required for accessibility but are visually hidden in `iconOnly` mode.
