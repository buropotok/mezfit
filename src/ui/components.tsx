...
export function TabsList({ className = '', children, style, ...props }: TabsListProps) {
  ...
  const updateIndicator = useCallback(() => {
    ...
    if (!list || !active || list.scrollWidth <= 0) { setClipPath('inset(.25rem 100% .25rem 0 round var(--ui-tab-radius))'); setIndicatorReady(false); return; }
  ...
  useEffect(() => {
    ...
    updateIndicator();
    const observer = new ResizeObserver(updateIndicator); observer.observe(list);
    const mutationObserver = new MutationObserver(updateIndicator); mutationObserver.observe(list, { subtree: true, attributes: true, attributeFilter: ['data-state'] });
