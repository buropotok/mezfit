...
import { createCoachProgram, getCoachPrograms, type ProgramListItem, type ProgramOwnerGroup, type ProgramStatus } from '../api';
...
import plusUrl from '../ui/icons/plus.svg';
...
function ProgramMenu({ program }: { program: ProgramListItem }) {
  ...
  return <Menu isOpen={open} onOpenChange={setOpen} align="end" label={`Действия с программой ${program.name}`} trigger={<IconButton className="programs-more" label={`Открыть меню программы ${program.name}`} data-no-dnd><MaskIcon src={dotsUrl} /></IconButton>}>
...
export function ProgramsPage({ initData, onSelectClient }: ProgramsPageProps) {
  ...
  const submitCreate = async () => {
    ...
    if (!name) { setCreateError('Укажите название программы'); return; }
    setCreating(true);
    setCreateError('');
    try {
      const { program } = await createCoachProgram(initData, name);
      setPrograms((current) => current ? [program, ...current] : [program]);
      setFilter('draft');
      setCreateOpen(false);
      setProgramName('');
    } catch (error) {
      setCreateError(error instanceof Error ? error.message : 'Не удалось создать программу');
    } finally {
      setCreating(false);
    }
  };
  ...
  return <section className="programs-page" aria-label="Программы">
    ...
    <FloatingActionButton label="Добавить программу" onClick={() => setCreateOpen(true)} disabled={programs === null}><MaskIcon src={plusUrl} /></FloatingActionButton>
    <Modal isOpen={createOpen} title="Новая программа" onClose={closeCreate} closeOnBackdrop={!creating} actions={[{ id: 'create', label: creating ? 'Создание…' : 'Создать', onClick: () => { void submitCreate(); }, disabled: creating || !programName.trim() }]}>
      <TextInput autoFocus label="Название программы" value={programName} maxLength={120} disabled={creating} error={createError || undefined} onChange={(event) => { setProgramName(event.currentTarget.value); if (createError) setCreateError(''); }} onKeyDown={(event) => { if (event.key === 'Enter' && !creating && programName.trim()) { event.preventDefault(); void submitCreate(); } }} />
    </Modal>
